  ...
  cost:
    name:
      exact:
        ...
        - kube_pod_labels
        - kube_pod_info
        # Label/annotation metrics emitted by the KubeState plugin for the
        # resources it supports (pods, namespaces, nodes). The plugin only emits
        # these when the corresponding insightsController.{labels,annotations}
        # toggle is enabled, so listing them here is additive. The webhook path
        # uses the cloudzero_ prefix below.
