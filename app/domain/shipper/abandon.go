...
func (m *MetricShipper) AbandonFiles(ctx context.Context, payload []*AbandonAPIPayloadFile) error {
	return m.metrics.SpanCtx(ctx, "shipper_AbandonFiles", func(ctx context.Context, id string) error {
		...
		// Send the request
		resp, err := m.SendHTTPRequest(ctx, "shipper_AbandonFiles_httpRequest", func() (*http.Request, error) {
			req, ierr := http.NewRequestWithContext(m.ctx, "POST", abandonEndpoint.String(), bytes.NewBuffer(enc))
			if err != nil {
