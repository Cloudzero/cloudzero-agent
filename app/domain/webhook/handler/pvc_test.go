...
func TestNewPersistentVolumeClaimHandler(t *testing.T) {
	...
	h := handler.NewPersistentVolumeClaimHandler(store, settings, types.TimeProvider(clock), &corev1.PersistentVolumeClaim{})
	assert.NotNil(t, h, "Handler should not be nil")
	assert.IsType(t, &hook.Handler{}, h, "Handler should be of type *hook.Handler")
}

func TestNewPersistentVolumeClaimConfigAccessor(t *testing.T) {
	settings := &config.Settings{
		Filters: config.Filters{
			Labels: config.Labels{
				Enabled: true,
	...
	accessor := handler.NewPersistentVolumeClaimConfigAccessor(settings)
	...
	t.Run("ResourceType", func(t *testing.T) {
		assert.Equal(t, config.PersistentVolumeClaim, accessor.ResourceType(), "ResourceType should return config.PersistentVolumeClaim")
	})
