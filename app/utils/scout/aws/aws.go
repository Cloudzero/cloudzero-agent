...
func (s *Scout) Detect(ctx context.Context) (types.CloudProvider, error) {
	req, err := http.NewRequestWithContext(ctx, httpMethodGET, metadataBaseURL+"/", nil)
	if err != nil {
	...
	if resp.StatusCode == http.StatusOK {
		...
		instanceReq, err := http.NewRequestWithContext(ctx, httpMethodGET, metadataBaseURL+"/instance-id", nil)
		if err != nil {
			return types.CloudProviderUnknown, nil
		}
...
func (s *Scout) getToken(ctx context.Context) (string, error) {
	req, err := http.NewRequestWithContext(ctx, httpMethodPUT, tokenURL, nil)
	if err != nil {
		return "", err
	}
...
func (s *Scout) getAccountID(ctx context.Context, token string) (string, error) {
	req, err := http.NewRequestWithContext(ctx, httpMethodGET, identityDocEndpoint, nil)
	if err != nil {
		return "", err
	}
