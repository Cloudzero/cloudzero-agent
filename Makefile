...
generate-changelog: ## Generate or update changelog for specified version (TAG_VERSION=1.2.3 make generate-changelog)
	...
	claude -p --dangerously-skip-permissions "CRITICAL: You are updating CloudZero Agent changelog for version $(TAG_VERSION). Follow these EXACT placement rules:\n\n1. ANALYZE: Read the existing CHANGELOG file structure carefully\n2. IDENTIFY: Locate the '### Major Bug Fixes Across X.Y.Z Series' section\n3. INSERT: Add new '#### $(TAG_VERSION) Fixes' subsection WITHIN that existing section\n4. UPDATE: Add new version to existing '## Version History' list\n5. NEVER: Add content after '## Upgrade Path' or create new major sections\n\nSpecific requirements:\n- Extract user-facing changes from git commits since last release\n- Place bug fixes under existing 'Major Bug Fixes Across X.Y.Z Series' section\n- Use format: '#### $(TAG_VERSION) Fixes' followed by bullet points\n- Add features under appropriate existing major sections\n- Update version history list with new version and date\n- Maintain exact existing document structure\n- Only modify $$CHANGELOG_FILE\n\nReference CLAUDE.md for complete formatting guidelines. CRITICAL: Do not add content after the Upgrade Path section."
